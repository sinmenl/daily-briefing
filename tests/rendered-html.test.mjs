...
test("renders the daily briefing", async () => {
  ...
  assert.match(html, /href="#creator-1">dbskill 更新至 v2.18.15/);
  assert.match(html, /href="#brief-1">Snapchat 不再推荐或奖励完全由 AI 生成的视频/);
  assert.match(html, /href="#brief-5">各大内容平台正在集体划定“AI 垃圾内容”边界/);
  assert.match(html, /href="#planet-1">大胡子：大多数人会死在第 3 层/);
  assert.match(html, /终局：变现能力 ≠ 赚钱能力。你赚得越急，死得越快/);
  assert.match(html, /id="hotlist-missing"/);
  assert.match(html, /7 月 31 日热点榜单未生成：昨日因额度不足主动停掉，不是系统异常/);
...
test("exports one page shell with cloud data for every date", async () => {
  ...
  assert.match(current, /大多数人都会死在了第3层/);
  ...
  assert.match(latest.mainHtml, /7 月 31 日热点榜单未生成：昨日因额度不足主动停掉，不是系统异常/);
  assert.match(latest.mainHtml, /dbskill 更新至 v2.18.15/);
  assert.match(latest.mainHtml, /网站开始需要同时服务人类用户和 AI Agent/);
  assert.match(latest.mainHtml, /大多数人都会死在了第3层/);
